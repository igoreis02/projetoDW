...
<head>
    ...
    <style>
        body { font-family: 'Inter', sans-serif; background-color: #f0f2f5; display: flex; flex-direction: column; align-items: center; min-height: 100vh; margin: 0; padding: 2rem 0; }
        .card { background-color: #ffffff; padding: 2.5rem; border-radius: 1rem; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); width: 95%; max-width: 1200px; margin-bottom: 2rem; text-align: center; }
        ...
        .close-btn, .back-btn-icon { position: absolute; top: 50%; transform: translateY(-50%); font-size: 2em; font-weight: bold; color: #aaa; text-decoration: none; }
        .close-btn { right: 0; }
        .back-btn-icon { left: 0; }
        .top-controls-container { width: 100%; display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem; flex-wrap: wrap; gap: 1rem; }
        .botao-adicionar-solicitacao { background-color: var(--cor-principal); color: white; padding: 12px 25px; font-size: 1.1em; border: none; border-radius: 5px; cursor: pointer; }
        ...
        /* ESTILOS ATUALIZADOS PARA PESQUISA E FILTROS DE STATUS */
        .container-pesquisa {
            display: flex;
            justify-content: center; /* Centraliza o campo */
            margin-bottom: 1rem;
            width: 100%;
        }
        .container-pesquisa input {
            ...
            width: 100%;
            max-width: 400px; /* Diminui a largura máxima do campo */
        }
        .status-filters {
            display: flex;
        ...
        .filter-container { margin-bottom: 2rem; padding-bottom: 1.5rem; border-bottom: 1px solid #e5e7eb; display: flex; flex-wrap: wrap; justify-content: center; gap: 0.8rem; min-height: 38px; }
        ...
        .solicitacoes-container { width: 100%; }
        ...
        .solicitacoes-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(350px, 1fr)); gap: 1.5rem; }
        .item-solicitacao.status-pendente { border-left-color: #3b82f6; }
        .item-solicitacao.status-em-andamento { border-left-color: #f59e0b; }
        .item-solicitacao.status-concluido { border-left-color: #22c55e; }
        .item-solicitacao.status-cancelado { border-left-color: #ef4444; }
        .item-solicitacao { background-color: #ffffff; border: 1px solid #e5e7eb; border-left-width: 5px; border-radius: 8px; padding: 1.5rem; box-shadow: 0 2px 8px rgba(0,0,0,0.06); display: flex; flex-direction: column; text-align: left; }
        .solicitacao-header { text-align: left; margin-bottom: 1rem; padding-bottom: 1rem; border-bottom: 1px dashed #d1d5db; width: 100%; }
        .solicitacao-header h3 { font-size: 1.3em; color: #111827; margin: 0; }
        .solicitacao-details { display: flex; flex-direction: column; gap: 0.8rem; flex-grow: 1; width: 100%; }
        ...
        .item-btn { padding: 6px 12px; font-size: 0.9em; font-weight: 600; border: none; border-radius: 6px; cursor: pointer; color: white; }
        .edit-btn { background-color: #3b82f6; }
        .concluir-btn { background-color: #22c55e; }
        .cancel-btn { background-color: #ef4444; }
        .status-tag { padding: 2px 8px; border-radius: 15px; font-weight: 600; font-size: 0.9em; color: white; }
        .status-pendente .status-tag { background-color: #3b82f6; }
        .status-em-andamento .status-tag { background-color: #f59e0b; }
        .status-concluido .status-tag { background-color: #22c55e; }
        .status-cancelado .status-tag { background-color: #ef4444; }
        ...
        .conteudo-modal { background-color: #fff; padding: 2rem; border-radius: 1rem; box-shadow: 0 5px 15px rgba(0, 0, 0, 0.3); width: 90%; max-width: 500px; }
        .fechar-modal { float: right; font-size: 2rem; font-weight: bold; cursor: pointer; }
        .formulario-modal label { display: block; margin-top: 1rem; font-weight: bold; color: #333; }
        .formulario-modal input, .formulario-modal select, .formulario-modal textarea { width: 100%; padding: 0.75rem; margin-top: 0.5rem; border-radius: 0.5rem; border: 1px solid #ccc; box-sizing: border-box; }
        .formulario-modal button { width: 100%; padding: 1rem; margin-top: 1.5rem; border-radius: 0.5rem; border: none; color: white; font-size: 1.1rem; cursor: pointer; display: flex; justify-content: center; align-items: center; }
        .formulario-modal .botao-salvar { background-color: #4CAF50; }
        .mensagem { margin-top: 1rem; padding: 1rem; border-radius: 0.5rem; text-align: center; }
        .spinner { border: 4px solid rgba(0,0,0,0.1); width: 36px; height: 36px; border-radius: 50%; border-left-color: #09f; animation: spin 1s ease infinite; margin: auto; }
    ...
    <div id="modalAdicionarSolicitacao" class="modal">
        <div class="conteudo-modal">
            ...
            <form id="formularioAdicionarSolicitacao" class="formulario-modal">
                ...
                <button type="submit" class="botao-salvar"><span>Adicionar</span></button>
            </form>
        </div>
    </div>
    <div id="modalEdicaoSolicitacao" class="modal">
        <div class="conteudo-modal">
            ...
            <form id="formularioEdicaoSolicitacao" class="formulario-modal">
                ...
                <select id="idUsuarioEdicao" name="id_usuario" required></select>
                <label for="idCidadeEdicao">Cidade:</label>
                <select id="idCidadeEdicao" name="id_cidade" required></select>
                ...
                <button type="submit" class="botao-salvar"><span>Salvar Alterações</span></button>
            </form>
        </div>
    </div>
